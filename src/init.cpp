  ...
  static SEXP run(SEXP samplerExpr, SEXP numIterExpr, SEXP isWarmupExpr, SEXP resultsTypeExpr)
  {
    ...
    for (int iter = 0; iter < numIter; ++iter) {
      ...
      if (resultsType == RESULTS_BOTH || resultsType == RESULTS_STAN) {
        ...
        // The random-effect scale's ridge is exactly traversable: rescaling
        // a block's tau and dividing that block's standardized effects by the
        // same factor leaves the linear predictor and the likelihood alone,
        // so the conditional along the curve is one-dimensional, log-concave,
        // and free of any data term. One slice draw per block per sweep, here
        // rather than inside the transition loop: it is the direction WALNUTS
        // cannot travel, and one exact draw of it is all that is wanted.
        // Sampling only - warmup runs AdaptiveWalnuts, which cannot be rebuilt
        // from a running state without resetting Adam and the mass estimator.
        if (!isWarmup && sampler.stanControl.ridge_move)
          sampler.paramSampler->ridgeMove();
        ...
        // sweep without changing the stored draw count. It is the escape
        // hatch for whatever the ridge move above does not reach - the
        // residual sd, which pays the two-block alternation whether or not
        // there is a random-effect block at all.

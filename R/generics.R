           ...
           ...)
{
  ...
  if (type %in% c("ev", "ppd")) {
    ...
    # ... with one wrinkle: an offset_type that SUBSTITUTES a component
    # ("bart"/"fixef"/"ranef") replaces that term with the length-n offset
    # vector, which carries no dim at all. Fall through to a term that still
    # holds the full array, or the result loses its dimensions - and with
    # them its identity as a draw matrix - whenever the substituted term is
    # the one the shape is read from. Only dim()/dimnames() are read here,
    # never the array itself: binding one would raise its reference count and
    # turn the dim-stripping below into the full-size copy this avoids.
    result_dim <- dim(indiv.bart)
    result_dimnames <- dimnames(indiv.bart)
    if (is.null(result_dim)) {
      result_dim <- dim(indiv.fixef)
      result_dimnames <- dimnames(indiv.fixef)
    }
    if (is.null(result_dim)) {
      result_dim <- dim(indiv.ranef)
      result_dimnames <- dimnames(indiv.ranef)
    }

# stan4bart store = 'trees' recompute seam (bart-train-recompute C3)
...
# store = 'trees' drops the bart blocks but keeps the parametric draws bit-identical
if (at_home()) local({
...
# recomputed BART surfaces match the stored path across readers (gaussian)
if (at_home()) local({
...
# recomputed BART surfaces match the stored path (binary)
if (at_home()) local({
...
# recomputed BART surfaces match the stored path for a factor-bearing bart component
if (at_home()) local({
...
# a store = 'trees' fit survives saveRDS/readRDS and fitted() works after reload
if (at_home()) local({
...
# store = 'trees' refuses a contradictory keepTrees = FALSE
if (at_home()) local({
...
# a large store = 'fits' fit nudges toward store = 'trees' once per session
local({
  ...
  # tinytest counterpart; collect what the expression emits and assert on that
  ...
  expect_true(any(grepl("store = \"trees\"",
                        messagesOf(stan4bart:::nudge_if_bart_store_large(2^31)))))
  expect_equal(length(messagesOf(stan4bart:::nudge_if_bart_store_large(2^31))), 0L)
  rm(list = ls(stan4bart:::.message_env), envir = stan4bart:::.message_env)
  expect_equal(length(messagesOf(stan4bart:::nudge_if_bart_store_large(2^20))), 0L)
})

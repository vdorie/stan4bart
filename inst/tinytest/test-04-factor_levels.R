...
# model fits with empty factor levels
...
# varcount is named per factor, not per level
...
# predict on fit-time test rows agrees with the fit-time test data
...
# a new BART factor level errors informatively at predict
local({
  df.newlevel <- df.test
...
# a new random-effects grouping level still predicts under both
# sample_new_levels settings.
...
# zero-ev random effects, per sample_new_levels. test-14-new_levels.R owns
# that behavior in detail; this only checks prediction still succeeds when
# the grouping level is new and the BART design carries a categorical factor.
local({
  df.newgroup <- df.test
